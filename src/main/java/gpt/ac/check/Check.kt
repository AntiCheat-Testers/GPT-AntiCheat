...
abstract class Check(val name: String, val description: String,val category:Category,val threshold:Long): Listener,
    ...
    open fun onFlying(
        ...
        if (perpetrator.location.subtract(0.0, 1.0, 0.0).block.type.isSolid) {
            this.validposition.remove(perpetrator, perpetrator.location)
...
open fun flag(increment:Long,player:Player){
    Thread{
        if(validposition[player]!=null) {
            ...
            PacketEvents.get().playerUtils.sendPacket(
                player,
                WrappedPacketOutEntityTeleport(player.entityId, validposition.get(player), false)
            )
