...
event.player.location.y=floor(event.player.location.y)
    ...
    override fun onFlying(
        ...
        if (!perpetrator.isDead) {
            ...
            if (predictedMotionY < -1.0457237242233686 && predictedMotionY < perpetrator.location.y - lastLoc.y &&


                !perpetrator.isFlying || (perpetrator.location.y - lastLoc.y).toFloat() == .42f && perpetrator.fallDistance > .42
            ) {
                flag(1, perpetrator)
                ...
                PacketEvents.get().playerUtils.sendPacket(
                    perpetrator,
                    WrappedPacketOutEntityTeleport(perpetrator.entityId,lastLoc, false)
                )
            }
        }
